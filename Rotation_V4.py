...
def ajout_planche(d, h, length, answer, Year):
    ...
    for k in range(len(answer) - 5, len(answer)):
        if k <= len(answer) - 5:
            answer[k] = str(datetime.strptime(answer[k] + '-' + str(Year), '%d-%m-%Y').date())
        elif k > len(answer) - 5 and datetime.strptime(answer[k] + '-' + str(Year), '%d-%m-%Y').date() >= get_time(answer[k-1]):
            answer[k] = str(datetime.strptime(answer[k] + '-' + str(Year), '%d-%m-%Y').date())
        elif k > len(answer) - 5 and datetime.strptime(answer[k] + '-' + str(Year), '%d-%m-%Y').date() < get_time(answer[k-1]):
            answer[k] = str(datetime.strptime(answer[k] + '-' + str(Year+1), '%d-%m-%Y').date())
...
def select_planche(n, l, d):
    ...
    for k in l:
        for i in range(1, len(k)):
            if int(k[i][0]) == n:
                if get_time(k[i][len(k[i])-5]).replace(day = 1) <= d < get_time(k[i][len(k[i])-1]).replace(day = 28) :
                    lgn += [k[i]]
    return lgn
...
def plot_ligne(n, l, d, i):
    ...
    for k in range(len(lgn)):
        for j in range(4):
            if get_time(lgn[k][j-4]).month == d.month and get_time(lgn[k][j-4]).year == d.year:
                intervention[j][k+1] = [get_time(lgn[k][j-4]).day]
        if get_time(lgn[k][len(lgn[k])-4]).replace(day = 1) <= d < get_time(lgn[k][len(lgn[k])-3]).replace(day = 28) and d.month != get_time(lgn[k][len(lgn[k])-3]).month:
            intervention[1][k+1] = (d + monthdelta(1) - d).days
            lab[k+1] = lgn[k][1]
        if get_time(lgn[k][len(lgn[k])-3]).replace(day = 1) <= d < get_time(lgn[k][len(lgn[k])-2]).replace(day = 28) and d.month != get_time(lgn[k][len(lgn[k])-2]).month:
            intervention[2][k+1] = (d + monthdelta(1) - d).days
            lab[k+1] = lgn[k][1]
        if get_time(lgn[k][len(lgn[k])-2]).replace(day = 1) <= d < get_time(lgn[k][len(lgn[k])-1]).replace(day = 28) and d.month != get_time(lgn[k][len(lgn[k])-1]).month:
            intervention[3][k+1] = (d + monthdelta(1) - d).days
            lab[k+1] = lgn[k][1]
        if get_time(lgn[k][len(lgn[k])-4]).replace(day = 1) <= d < get_time(lgn[k][len(lgn[k])-1]).replace(day = 28):
            lab[k+1] = lgn[k][1]
    ...
    ct = 0
    for k in range(len(lgn)):
        if intervention[1][k+1-ct] == 0 and intervention[2][k+1-ct] == 0 and intervention[3][k+1-ct] == 0:
            del x[k+1-ct]
            del width[k+1-ct]
            del lab[k+1-ct]
...
def tot_legume(l, d, h, leg):
    ...
    for k in range(9):
        ...
        for i in lgn:                     
            tp = [get_time(i[-5]).replace(day = 1), (get_time(i[-4]) + monthdelta(1)).replace(day = 1)]
            tp2 = [get_time(i[-4]).replace(day = 1), (get_time(i[-3]) + monthdelta(1)).replace(day = 1)]
            tp3 = [get_time(i[-2]).replace(day = 1), (get_time(i[-1]) + monthdelta(1)).replace(day = 1)]
            
            if i[1] == leg:
                info = i
                if get_time(i[-4]) != get_time(i[-5]) and tp[0] <= d < tp[1]:
                    ct += 1
                    if d.month == get_time(i[-5]).month and d.month != get_time(i[-4]).month:
                        dnb = (((d + monthdelta(1)).replace(day = 1) -get_time(i[-5])).days - 1) / (get_time(i[-4]) - get_time(i[-5])).days
                    elif d.month != get_time(i[-5]).month and d.month == get_time(i[-4]).month:
                        dnb = (get_time(i[-4]) - d.replace(day = 1)).days / (get_time(i[-4]) - get_time(i[-5])).days
                    else:
                        dnb = (d + monthdelta(1) - d).days / (get_time(i[-4]) - get_time(i[-5])).days
                        
                elif get_time(i[-4]) == get_time(i[-5]) and tp2[0] <= d < tp2[1]:
                    ct += 1
                    if d.month == get_time(i[-4]).month and d.month != get_time(i[-3]).month:
                        dnb = (((d + monthdelta(1)).replace(day = 1) -get_time(i[-4])).days - 1) / (get_time(i[-3]) - get_time(i[-4])).days
                    elif d.month != get_time(i[-4]).month and d.month == get_time(i[-3]).month:
                        dnb = (get_time(i[-3]) - d.replace(day = 1)).days / (get_time(i[-3]) - get_time(i[-4])).days
                    else:
                        dnb = (d + monthdelta(1) - d).days / (get_time(i[-3]) - get_time(i[-4])).days
                        
                elif tp3[0] <= d < tp3[1]:
                    ct2 += 1
                    if d.month == get_time(i[-2]).month and d.month != get_time(i[-1]).month:
                        dnb = (((d + monthdelta(1)).replace(day = 1) -get_time(i[-2])).days - 1) / (get_time(i[-1]) - get_time(i[-2])).days
                    elif d.month != get_time(i[-2]).month and d.month == get_time(i[-1]).month:
                        dnb = (get_time(i[-1]) - d.replace(day = 1)).days / (get_time(i[-1]) - get_time(i[-2])).days
                    else:
                        dnb = (d + monthdelta(1) - d).days / (get_time(i[-1]) - get_time(i[-2])).days
               
    result = [0, 0]
 
    if ct != 0:
        totli = 20/(float(info[h.index('intra_rang')])*float(info[h.index('taux_germ')])*float(info[h.index('taux_reprise')]))
        totpl = int(info[h.index('Nb_rangs')])*totli
        totplmois = int(totpl*dnb)
        
        result[0] = ct*totplmois
        
    if ct2 != 0:
        totli = 20/(float(info[h.index('intra_rang')]))
        totpl = int(info[h.index('Nb_rangs')])*totli
        totplmois = totpl*dnb*float(info[h.index('prod_plant')])
        
        result[1] = ct2*totplmois
...
class Block:
    ...
    def __init__(self, win):
        ...
        self.L = 120
        self.Year = [2022, 2023, 2024]
        self.header = ["ligne", "nom", "x_planche", "l_planche", "Nb_rangs", "inter_rang", "intra_rang", "déphasage", "prod_plant", "taux_germ", "taux_reprise", "d_cycle_serre", "d_cycle_champ", "f_semis", "d_récolte", "f_cylce"]
        self.dir = "C:/Users/tigerault\OneDrive - agroparistech.fr/Documents - Académie Yamashita/Construction du modèle pédagogique/Enseignement agronomique/Modèle agronomique/Cartographie/Python/Rotation Maraîchage"
        self.date = datetime.strptime("01-01-" + str(self.Year[0]), '%d-%m-%Y').date()
        self.p = 1
        ...
        self.lb4 = Label(win, text = "Ajout légume")
        ...
        self.btn4=Button(window, text="Opérations", command = self.plot_serre)
        ...
        self.lbl2.place(x=1635, y=400)
        
        self.lstb2 = Listbox(win)
        self.lstb2.bind("<Double-Button-1>", self.plot_maille)
        self.lstb2.place(x = 1600, y = 430)
        
        self.lbly=Label(win, text='Année')
        self.lbly.place(x = 275, y=20)
        
        self.lstby = Listbox(win, width = 5, height = 4)
        self.lstby.bind("<Double-Button-1>", self.plot_Year)
        self.lstby.place(x = 275, y = 50)
        for k in range(len(self.Year)):
            self.lstby.insert(k+1, self.Year[k])
    ...
    def updateTime(self, event):
        self.date = self.date.replace(year = self.lstby.get(ACTIVE))
    ...
    def add(self):
        ...
        result = ajout_planche(self.dir, self.header, self.L, [k.get() for k in self.t], self.lstby.get(ACTIVE))
    ...
    def plot_serre(self):
        ...
        for k in range(12):
            dtp = get_time(str(self.date.year) + '-' + str(k+1) + '-01')
        ...
        window2.title('Plannification des opérations')
    ...
    def plot_maille(self, event):
        ...
        window3.geometry("1920x1080+30+30")
        window3.mainloop()
        
    def plot_Year(self, event):
    ...
    def occupe(self, event):
        ...
        if g in [str(k+1) for k in range(9)]:
            ...
            for k in sorted([float(li[i][2]) for i in range(len(li))]):
                self.plot1.text(x = -17, y = k, s = str(k))
            self.canvas.draw() 
...
class Serre:
    ...
    def __init__(self, win2, x, leg):
        ...
        self.init_plot()
        self.update_plot_semis(x)
        self.update_plot_recolte(x)
    ...
    def update_plot_semis(self, x):
        self.semis = []
        for k in range(len(x)):
            self.semis += [[]]
            for j in range(len(x[k])):
                self.semis[k] += [x[k][j][0]]
                
        m = [(get_time('2022-01-01')+monthdelta(k)).strftime("%B")[:3] for k in range(12)]
        self.plot1.bar(m, self.semis[0], label = self.legumes[0])
        
        bot = [0 for k in range(12)]
        for k in range(len(self.semis)-1):
            for i in range(12):
                bot[i] += self.semis[k][i]
            self.plot1.bar(m, self.semis[k+1], bottom = bot, label = self.legumes[k+1])
            
        self.plot1.set_ylabel('Semis mensuels')
        self.plot1.legend()
        
        self.canvas.draw()
        
    def update_plot_recolte(self, x):
        self.recolte = []
        for k in range(len(x)):
            self.recolte += [[]]
            for j in range(len(x[k])):
                self.recolte[k] += [x[k][j][1]]
                
        m = [(get_time('2022-01-01')+monthdelta(k)).strftime("%B")[:3] for k in range(12)]
        self.plot2.bar(m, self.recolte[0], label = self.legumes[0])
        for index, value in enumerate(self.recolte[0]):
            if value != 0:
                self.plot2.text(index, value, str(round(value)))
        
        bot = [0 for k in range(12)]
        for k in range(len(self.recolte)-1):
            for i in range(12):
                bot[i] += self.recolte[k][i]
            self.plot2.bar(m, self.recolte[k+1], bottom = bot, label = self.legumes[k+1])
            for index, value in enumerate(self.recolte[k+1]):
                if value != 0:
                    self.plot2.text(index-1, value + bot[index], str(round(value)))
        
        self.plot2.set_ylabel('Récolte mensuelle')
        self.plot2.legend()
        self.canvas.draw()
...
class Maille:
    ...
    def __init__(self, win, combi, act):
        ...
        self.lbl1=Label(win, text='Calculateur', font=("Helvetica", 14))
        self.lbl1.place(x=145, y=30)
        
        self.lbl2=Label(win, text='Production attendue')
        self.lbl2.place(x=175, y=80)
        
        self.t1 = Entry(win, bd=2, width = 15)
        self.t1.place(x = 180, y = 100)
        
        self.t2 = Entry(win, bd=2, width = 15)
        self.t2.place(x = 120, y = 230)
        
        self.t3 = Entry(win, bd=2, width = 15)
        self.t3.place(x = 120, y = 360)
        
        self.lstb1 = Listbox(win, width = 10, height = 4)
        self.lstb1.bind("<Double-Button-1>", self.calculateur)
        self.lstb1.place(x = 55, y = 80)
        for k in range(len(self.act)):
            self.lstb1.insert(k+1, self.act[k])
    ...
    def update_plot(self):
        self.densite = [0 for k in self.info]
        m = max([float(k[6]) for k in self.info])
        for k in self.info:
            self.plot.set_xlim(0, float(k[3]))
            x, y = [], []
            for j in range(int(k[4])):
                for i in range(6*int(m/float(k[6]))):
            ...
            self.densite[self.info.index(k)] = str(round(0.5*int(k[4])/float(k[6]), 1))
            self.plot.plot(x, y, marker=".", markersize=20, linestyle = 'None', label = k[1] + ' ' + self.densite[self.info.index(k)] + '/m2')
            self.plot.set_ylim(min(y), max(y))
        self.plot.legend()
        self.canvas.draw()
        
    def calculateur(self, event):
        self.t2.delete(0, END)
        self.t3.delete(0, END)
        for k in range(len(self.info)):
            if self.info[k][1] == self.lstb1.get(ACTIVE):
                if self.t1.get() == '':
                    self.t2.insert('Quantité Invalide')
                    self.t3.insert('Quantité Invalide')
                else :
                    nb_pl = int((float(self.t1.get())/float(self.info[k][8]))/(20.0*float(self.info[k][3])*float(self.densite[k]))) + 1
                    self.t2.insert(0, nb_pl)
                    self.t3.insert(0, int((nb_pl*(20.0*float(self.info[k][3])*float(self.densite[k])))/ (float(self.info[k][9])*float(self.info[k][10]))))
...
window.geometry("1030x720+30+30")
window.mainloop()
